...
class ManifestoTopicModeling:
	...
	def perform_topic_modeling(self, manifesto_corpus: List[str], party_names: List[str]) -> Tuple[
		...
		# Rulez și LDA pe aceleași date:
		self.lda_topic_modeling(all_chunks)

		return self.topic_model, party_topics, topic_df

    def lda_topic_modeling(self, all_chunks: List[str], num_topics: int = 12):
        from gensim.corpora import Dictionary
        ...
        # Construim dictionary si corpus
        id2word = Dictionary(tokenized_chunks)
        corpus = [id2word.doc2bow(text) for text in tokenized_chunks]
        ...
        # Afisam topicele cu nume daca sunt definite
        print("\nLDA Topics (cu nume daca sunt definite):")
        for i, topic in lda_model.show_topics(num_topics=num_topics, formatted=False):
            topic_words = [word for word, _ in topic[:5]]
            try:
                topic_label = TopicTheme.get_name(i)
            except Exception:
                topic_label = f"Topic {i}"
            print(f"{topic_label}: {', '.join(topic_words)}")

        # Coerenta
        coherence_model_lda = CoherenceModel(model=lda_model, texts=tokenized_chunks, dictionary=id2word, coherence='c_v')
